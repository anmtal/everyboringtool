...
export function createEngine(dict) {
  ...
  function suggest(original) {
    ...
    // Edits-2 is O(n²) in probes, so it is strictly bounded: only for short
    // words, and with a low probe ceiling. Long unknown words (names, foreign
    // text) blew past 20 s of main-thread time before this guard.
    if (found.length < 4 && w.length <= 9) {
      let guard = 0;
      outer: for (const c1 of e1) {
        for (const c2 of edits1(c1)) {
          if (++guard > 20000) break outer;
  ...
  function analyze(text, opts) {
    ...
    // URLs, e-mail addresses and bare domains are not prose. Blank them out with
    // spaces of exactly the same length — every offset below still points into
    // the original `text` — so their pieces are neither spell-checked nor read as
    // sentence boundaries ("example.com/Foo" is not a missing capital).
    // The two whitespace rules below deliberately keep scanning the real `text`,
    // since the blanks themselves would otherwise look like stray spaces.
    const src = text.replace(
      /https?:\/\/\S+|\S+@\S+\.\S+|\b[\w-]+\.(?:com|org|net|io|co|edu|gov|app|dev)\b/gi,
      (m) => " ".repeat(m.length)
    );
    ...
    // sentence capitalisation
    {
      ...
      let m;
      while ((m = re.exec(src))) {
        // Dots are kept in the look-behind and then stripped, so the internal
        // dots of "i.e.", "e.g." and "U.S." fold onto the ABBREV entries
        // ("ie", "eg") instead of leaving only the final letter.
        const before = src.slice(Math.max(0, m.index - 6), m.index).match(/([A-Za-z][A-Za-z.]*)$/);
        if (before && ABBREV.has(before[1].replace(/\./g, "").toLowerCase())) continue;
        const at = m.index + m[1].length + m[2].length;
        if (overlaps(at, at + 1)) continue;
        const sw = src.slice(at).match(/^[A-Za-z'’]+/);
      ...
      const first = src.match(/^(\s*)([a-z])/);
      if (first) {
        const at = first[1].length;
        const fw = src.slice(at).match(/^[A-Za-z'’]+/);
    ...
    // lowercase pronoun "i"
    {
      ...
      while ((m = re.exec(src))) {
        if (m[0][0] !== "i") continue;
        if (/^i\.e/i.test(src.slice(m.index, m.index + 3))) continue; // the "i" of "i.e." is not the pronoun
        if (overlaps(m.index, m.index + m[0].length)) continue;
    ...
    while ((wm = wordRe.exec(src))) {
      ...
      const prevChar = src.slice(0, start).match(/(\S)\s*$/);
      const sentenceStart = !prevChar || /[.!?]/.test(prevChar[1]);
      if (ignoreNames && /^[A-Z][a-z]/.test(raw) && !sentenceStart) continue;
      // Suggestions are the expensive part, so only the first 25 misspellings get
      // them up front. The rest carry `suggestions: null`, meaning "not computed
      // yet" — the UI offers a button that calls suggest() for that one word.
      const capFirst = sentenceStart && /^[a-z]/.test(raw);
      let sg = null;
      if (checked < 25) {
        sg = suggest(raw);
        if (capFirst) sg = sg.map((s) => s.charAt(0).toUpperCase() + s.slice(1));
      }
      push({ id: ++ID, type: "spelling", start, end, bad: raw, fix: null, capFirst, message: "Possible spelling mistake.", suggestions: sg });
      checked++;
    }
