...
function familyFor(categorySlug, name, slug) {
  ...
  if (/\bscreen\b/.test(n) && !/screenshot|screencast/.test(n)) return "screen";
...
function verbFor(name, family) {
  ...
  if (family === "screen") return "Go full screen";
...
export default function ToolSkeleton({ slug }) {
  ...
  if (family === "file") {
    ...
    // e.g. White Screen — no input, just a full-screen button.
    fields = null;
  } else {
    // family === "pair"
