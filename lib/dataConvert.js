...
export function csvToMarkdown(input) {
  const rows = parseCsv(input);
  if (rows.length === 0) throw new Error("Paste some CSV to convert.");
...
// ---- CSV -> HTML table ----
export function csvToHtml(input) {
  const rows = parseCsv(input);
  if (rows.length === 0) throw new Error("Paste some CSV to convert.");
...
export function htmlToText(input) {
  const s = (input || "").trim();
  if (!s) throw new Error("Paste some HTML to convert.");
  const doc = new DOMParser().parseFromString(s, "text/html");
  doc.querySelectorAll("script, style, noscript").forEach((n) => n.remove());
  doc.querySelectorAll("br").forEach((b) => b.replaceWith("\n"));
  doc.querySelectorAll("p, div, li, tr, h1, h2, h3, h4, h5, h6, blockquote, section, article, header, footer").forEach((b) => b.append("\n"));
  const text = (doc.body ? doc.body.textContent : doc.textContent) || "";
  return text.replace(/[ \t]+/g, " ").replace(/ *\n */g, "\n").replace(/\n{3,}/g, "\n\n").trim();
}

// ---- TSV <-> CSV ----
export function tsvToCsv(input) {
  const s = String(input || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").trim();
  if (!s) throw new Error("Paste some TSV to convert.");
  return s.split("\n").map((line) => line.split("\t").map(csvCell).join(",")).join("\n");
}
export function csvToTsv(input) {
  const rows = parseCsv(input);
  if (rows.length === 0) throw new Error("Paste some CSV to convert.");
...
// ---- HTML table -> CSV ----
export function htmlTableToCsv(input) {
  const s = (input || "").trim();
  if (!s) throw new Error("Paste HTML that contains a table.");
  const doc = new DOMParser().parseFromString(s, "text/html");
  const table = doc.querySelector("table");
  if (!table) throw new Error("No <table> element found in the HTML.");
