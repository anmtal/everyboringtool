...
// Hate-speech / strong-slur blocklist. The public dictionary is a standard
// Scrabble word list, which legitimately contains racial, ethnic and homophobic
// slurs. Those have no place in a family-friendly tool and — once ads go live —
// would place a Google ad next to derogatory content (an AdSense Publisher
// Policies breach). They are stripped from every result set at load. Kept to
// exact, unambiguous slurs (whole-word match only) so ordinary words — including
// harmless homographs like "spice"/"spicy" — and mild profanity that mainstream
// word-finders legitimately list are never touched (avoids the Scunthorpe trap).
const BLOCKED = new Set([
  "nigger", "niggers", "nigga", "niggas", "niggaz", "niggah", "niggahs",
  "faggot", "faggots", "faggy", "faggoty",
  "kike", "kikes",
  "spic", "spics",
  "wetback", "wetbacks",
  "beaner", "beaners",
  ...
  "abbo", "abbos",
  "boong", "boongs",
  "golliwog", "golliwogs", "golliwogg", "golliwoggs",
  "negress", "negresses",
]);
...
export function matchPattern(raw) {
  ...
  if (!pat || !/[a-z]/.test(pat.replace(/-/g, ""))) return [];
