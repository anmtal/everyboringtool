...
export default function PdfToPng() {
  ...
  async function onFile(e) {
    ...
    setBusy(true); setError(""); revokeAll(); setPages([]);
  ...
  function downloadAll() {
    pages.forEach((pg, i) => setTimeout(() => {
      const a = document.createElement("a");
      a.href = pg.url; a.download = `${name}-page-${pg.num}.png`;
  ...
  return (
    <div className="tool">
      <div className="dropzone" role="button" tabIndex={0} aria-disabled={busy}
        ...
        <p className="dropzone-sub">Each page becomes a PNG image — all in your browser, nothing uploaded.</p>
      </div>
      ...
      {pages.length > 0 && (
        <>
          <div className="tool-actions">
            <button type="button" className="btn btn-primary" onClick={downloadAll}>↓ Download all {pages.length} PNG{pages.length === 1 ? "" : "s"}</button>
          </div>
          <div className="grid" style={{ marginTop: 4 }}>
            {pages.map((pg) => (
              <div key={pg.num} className="card" style={{ padding: 12 }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={pg.url} alt={`Page ${pg.num}`} loading="lazy" style={{ width: "100%", height: "auto", borderRadius: 6, marginBottom: 8 }} />
                <div className="tool-actions" style={{ justifyContent: "space-between" }}>
                  <span className="tool-note">Page {pg.num}</span>
                  <a className="btn btn-success" href={pg.url} download={`${name}-page-${pg.num}.png`}>Download</a>
                </div>
              </div>
            ))}
          </div>
